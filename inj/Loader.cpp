...
int main()
{
	...
	DWORD threadId;
	DWORD processId = GetProcessInformationByWindow(xor ("Notepad"), &threadId); // WRITE WINDOW NAME for proccess  <------

	if (processId == 0 && threadId == 0)
	{
		for (;; Sleep(100))
		{
			processId = GetProcessInformationByWindow(xor ("Notepad"), &threadId);  // WRITE WINDOW NAME for proccesss <------
